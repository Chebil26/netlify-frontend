...
export const readingChallengeByUser = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_SERVER}/api/challenges/reading-challenge/`,
      config
    );
...
export const createReadingChallenge = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.REACT_APP_API_SERVER}/api/challenges/reading-challenge-create/`,
      {},
      config
    );
...
export const updateReadingChallenge =
  (readingChallenge) => async (dispatch, getState) => {
    try {
      dispatch({
      ...
      const { data } = await axios.put(
        `${process.env.REACT_APP_API_SERVER}/api/challenges/reading-challenge-update/`,
        readingChallenge,
        config
      );
      dispatch({
...
export const incrementReadingChallenge =
  (readingChallenge) => async (dispatch, getState) => {
    try {
      dispatch({
      ...
      const { data } = await axios.put(
        `${process.env.REACT_APP_API_SERVER}/api/challenges/reading-challenge-increment/`,
        readingChallenge,
        config
      );
      dispatch({
